...
### 2.6 AI Attribution (Strictly Forbidden)

**NEVER MENTION AI IN COMMITS OR CODE**

- Never mention "Claude", "AI", "assistant", "generated", or "Anthropic" in commit messages
- Never mention AI tools in PR descriptions or code comments
- Write commit messages as if a human developer made the changes
- No AI attribution comments in code (`// Generated by...`, `// AI-assisted...`)
- No AI references in documentation or changelogs

**Rationale:** Code ownership and accountability. All code is reviewed and approved by human developers who take responsibility for it.

**Enforcement:**

- Pre-commit hooks will block commits containing AI references
- PR reviews will reject AI attribution
- CI may include attribution guards
...
- `links`: Link cards and link data
...
git commit -m "a11y(links): add aria-labels to icon-only buttons"
git commit -m "seo: add structured data for person schema"
```

**Detailed Commit Examples (with body):**

```
feat(links): add professional links section with icons

- Added LinkCard component with hover states
- Integrated react-icons for platform logos
- Implemented responsive grid layout
- Added dark mode support for all cards
...
Skip link was not receiving focus on tab. Updated CSS
to ensure proper visibility and focus management.

- Fixed skip-link z-index conflict
...
```
perf(image): optimize avatar and OG images

Converted all images to WebP format and added
proper srcset for responsive loading.

- Avatar: 48KB → 12KB (75% reduction)
- OG image: 180KB → 45KB (75% reduction)
- Added blur placeholder for LCP improvement

Before: LCP 2.8s → After: LCP 1.9s
```

```
refactor(ui): extract shared button styles to component

Consolidated duplicate button styling across LinkCard,
Header, and Footer into reusable Button component.
...
| ----------- | ------------------------- | ------------------------------ |
| `feature/`  | New functionality         | `feature/social-links`         |
| `fix/`      | Bug fixes                 | `fix/dark-mode-contrast`       |
| `perf/`     | Performance improvements  | `perf/image-optimization`      |
| `a11y/`     | Accessibility fixes       | `a11y/keyboard-navigation`     |
| `seo/`      | SEO improvements          | `seo/structured-data`          |
| `refactor/` | Code refactoring          | `refactor/component-structure` |
| `hotfix/`   | Critical production fixes | `hotfix/broken-links`          |
...
- Never use generic names (`feature/update`, `fix/bug`)

**Examples:**

```bash
# Good
git checkout -b feature/github-link-card
git checkout -b fix/mobile-menu-overflow
git checkout -b a11y/skip-navigation-link
git checkout -b perf/lazy-load-icons

# Bad
git checkout -b update
git checkout -b fix
git checkout -b feature/stuff
git checkout -b Feature/NewThing  # No capitals
```
...
# Use PR template (see Section 20)
...
### 3.7 Branch Protection (Enforced)

**Main Branch Rules:**

| Rule                      | Setting | Rationale                 |
| ------------------------- | ------- | ------------------------- |
| Require PR before merge   | Yes     | No direct commits to main |
| Required reviewers        | 1       | Code review mandatory     |
| Dismiss stale reviews     | Yes     | Re-review after changes   |
| Require CODEOWNERS review | Yes     | Owner accountability      |
